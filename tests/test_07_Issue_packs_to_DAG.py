...
# Test name: t07 Issue packs to DAG
import pytest
import time
import json
...
class Test_07_Issue_packs_to_DAG:
  ...
  def test_07_Issue_packs_to_DAG(self):
    ...
    self.driver.find_element(By.NAME, "cat_id").send_keys("packs7")
    ...
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=configure_edit\"][href*=\"cat_id=packs7\"]")) > 0
    self.driver.execute_script("//SETDESC:Assert category is saved")
    self.driver.find_element(By.XPATH, "//td//span[contains(.,'packs7')]").send_keys("SAVESCREENSHOT")
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packrcpt")) > 0
    assert len(self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packissue")) > 0
    self.driver.execute_script("//SETDESC:Assert options to acknowledge packs as received and issue packs to DAGs are present")
    self.driver.find_element(By.CSS_SELECTOR, ".packmgmt-packrcpt").send_keys("SAVESCREENSHOT")
    ...
    self.driver.find_element(By.ID, "user_role").find_element(By.XPATH, "(descendant::option)[. = 'PackIssue']").click()
    ...
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"prefix=pack_management\"][href*=\"page=packs\"]").click()
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs7\"]")) > 0
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs7\"]").click()
    assert len(self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packrcpt")) == 0
    assert len(self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packissue")) > 0
    self.driver.execute_script("//SETDESC:Assert option to issue packs to DAGs is present")
    self.driver.find_element(By.CSS_SELECTOR, ".packmgmt-packissue").send_keys("SAVESCREENSHOT")
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packrcpt")) > 0
    assert len(self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packissue")) == 0
    self.driver.execute_script("//SETDESC:Assert option to acknowledge packs as received is present")
    self.driver.find_element(By.CSS_SELECTOR, ".packmgmt-packrcpt").send_keys("SAVESCREENSHOT")
