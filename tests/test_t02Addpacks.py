...
class TestT02Addpacks():
  ...
  def test_t02Addpacks(self):
    ...
    self.driver.find_element(By.CSS_SELECTOR, "input[type=\"submit\"]").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    elements = self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=configure_edit\"][href*=\"cat_id=packs2\"]")
    assert len(elements) > 0
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"prefix=pack_management\"][href*=\"page=packs\"]").click()
    elements = self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=packs_add\"][href*=\"cat_id=packs2\"]")
    assert len(elements) > 0
    ...
    assert len(elements) > 0
    elements = self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"pack_id\"][value=\"3\"]")
    assert len(elements) > 0
    ...
    WebDriverWait(self.driver, 30).until(expected_conditions.invisibility_of_element_located((By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs2\"]")))
    elements = self.driver.find_elements(By.XPATH, "//p[contains(.,\'Add Packs\')]")
    assert len(elements) == 0
