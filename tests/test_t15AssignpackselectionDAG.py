...
class TestT15AssignpackselectionDAG():
  ...
  def test_t15AssignpackselectionDAG(self):
    ...
    self.driver.find_element(By.CSS_SELECTOR, "input[type=\"submit\"]").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    elements = self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=configure_edit\"][href*=\"cat_id=packs15\"]")
    assert len(elements) > 0
    ...
    elements = self.driver.find_elements(By.CSS_SELECTOR, "select[name=\"pack_id\"][data-optcount=\"1\"]")
    assert len(elements) > 0
    ...
    self.driver.execute_script("$(\'#south\').remove()")
    self.driver.find_element(By.CSS_SELECTOR, "input[name=\"pack_id\"][value=\"1\"]").click()
    elements = self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packrcpt input[type=\"submit\"]:not([disabled])")
    assert len(elements) == 0
    self.driver.find_element(By.CSS_SELECTOR, "input[name=\"pack_id\"][value=\"2\"]").click()
    elements = self.driver.find_elements(By.CSS_SELECTOR, ".packmgmt-packrcpt input[type=\"submit\"]:not([disabled])")
    assert len(elements) > 0
    self.driver.find_element(By.CSS_SELECTOR, ".packmgmt-packrcpt input[type=\"submit\"]").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    elements = self.driver.find_elements(By.CSS_SELECTOR, ".mod-packmgmt-okmsg")
    assert len(elements) > 0
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"DataEntry/record_status_dashboard.php\"]").click()
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"page=visit_lab_data\"]").click()
    self.driver.execute_script("$(\'[name=\"pack_id\"]\').attr(\'data-optcount\',\'\'+$(\'[name=\"pack_id\"] option\').length)")
    elements = self.driver.find_elements(By.CSS_SELECTOR, "select[name=\"pack_id\"][data-optcount=\"3\"]")
    assert len(elements) > 0
    elements = self.driver.find_elements(By.CSS_SELECTOR, "select[name=\"pack_id\"] option[value=\"1\"]")
    assert len(elements) > 0
    elements = self.driver.find_elements(By.CSS_SELECTOR, "select[name=\"pack_id\"] option[value=\"2\"]")
    assert len(elements) > 0
    elements = self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"pack_count\"][value=\"\"]")
    assert len(elements) > 0
    elements = self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"pack_date\"][value=\"\"]")
    ...
    assert len(elements) > 0
    elements = self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"pack_count\"][value=\"1\"]")
    assert len(elements) > 0
    elements = self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"pack_date\"][value=\"\"]")
    assert len(elements) == 0
