...
# Test name: t14 Assign pack selection
import pytest
import time
import json
...
class Test_14_Assign_pack_selection:
  ...
  def test_14_Assign_pack_selection(self):
    ...
    self.driver.find_element(By.NAME, "cat_id").send_keys("packs14")
    ...
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=configure_edit\"][href*=\"cat_id=packs14\"]")) > 0
    self.driver.execute_script("//SETDESC:Assert category is saved")
    self.driver.find_element(By.XPATH, "//td//span[contains(.,'packs14')]").send_keys("SAVESCREENSHOT")
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "select[data-optcount=\"5\"]")) > 0
    self.driver.execute_script("//SETDESC:Assert dropdown contains 4 packs")
    self.driver.find_element(By.CSS_SELECTOR, "select[data-optcount=\"5\"]").send_keys("SAVESCREENSHOT")
    ...
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "input[name=\"pack_date\"][value=\"\"]")) > 0
    self.driver.execute_script("//SETDESC:Assert no pack assigned")
    self.driver.find_element(By.CSS_SELECTOR, "input[name=\"pack_id\"]").send_keys("SAVESCREENSHOT")
    self.driver.execute_script("//SAVEDESC:Delete form")
    self.driver.execute_script("$('#south').remove();dataEntrySubmit('submit-btn-deleteform')")
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
