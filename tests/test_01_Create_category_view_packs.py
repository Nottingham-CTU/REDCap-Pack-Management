...
# Test name: t01 Create category / view packs
import pytest
import time
import json
...
class Test_01_Create_category_view_packs:
  ...
  def test_01_Create_category_view_packs(self):
    ...
    self.driver.find_element(By.NAME, "cat_id").send_keys("packs1")
    ...
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=configure_edit\"][href*=\"cat_id=packs1\"]")) > 0
    self.driver.execute_script("//SETDESC:Assert category is saved")
    self.driver.find_element(By.XPATH, "//td//span[contains(.,'packs1')]").send_keys("SAVESCREENSHOT")
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"prefix=pack_management\"][href*=\"page=packs\"]").click()
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs1\"]")) > 0
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs1\"]").click()
    assert len(self.driver.find_elements(By.XPATH, "//p[contains(.,'View Packs')]")) > 0
    self.driver.execute_script("//SETDESC:Assert can access pack list")
    self.driver.find_element(By.XPATH, "//p[contains(.,'View Packs')]").send_keys("SAVESCREENSHOT")
    ...
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, "a[href*=\"prefix=pack_management\"][href*=\"page=packs\"]")))
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"prefix=pack_management\"][href*=\"page=packs\"]").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, "a[href*=\"page=packs_add\"][href*=\"cat_id=packs1\"]")))
    assert len(self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs1\"]")) == 0
    self.driver.execute_script("//SETDESC:Assert cannot access pack list")
    self.driver.find_element(By.XPATH, "//table[@class='mod-packmgmt-listtable']//tr[contains(.,'packs1')]//td[2]").send_keys("SAVESCREENSHOT")
    self.driver.execute_script("window.location.href=$('a[href*=\"page=packs_add\"][href*=\"cat_id=packs\"]').attr('href').replace('page=packs_add','page=packs_list')")
    None if len(elements := self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=packs_add\"][href*=\"cat_id=packs1\"]")) == 0 else WebDriverWait(self.driver, 30).until(expected_conditions.staleness_of(elements[0]))
    assert len(self.driver.find_elements(By.XPATH, "//p[contains(.,'View Packs')]")) == 0
