...
class TestT01Createcategoryviewpacks():
  ...
  def test_t01Createcategoryviewpacks(self):
    ...
    self.driver.find_element(By.CSS_SELECTOR, "input[type=\"submit\"]").click()
    WebDriverWait(self.driver, 30).until(expected_conditions.presence_of_element_located((By.ID, "south")))
    elements = self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=configure_edit\"][href*=\"cat_id=packs1\"]")
    assert len(elements) > 0
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"prefix=pack_management\"][href*=\"page=packs\"]").click()
    elements = self.driver.find_elements(By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs1\"]")
    assert len(elements) > 0
    self.driver.find_element(By.CSS_SELECTOR, "a[href*=\"page=packs_list\"][href*=\"cat_id=packs1\"]").click()
    elements = self.driver.find_elements(By.XPATH, "//p[contains(.,\'View Packs\')]")
    assert len(elements) > 0
    ...
    self.driver.execute_script("$(\'#notify_email_role\').prop(\'checked\',false)")
    dropdown = self.driver.find_element(By.ID, "user_role")
    dropdown.find_element(By.XPATH, "//option[. = 'PackAdd']").click()
    self.driver.find_element(By.ID, "assignDagRoleBtn").click()
    ...
    WebDriverWait(self.driver, 30).until(expected_conditions.invisibility_of_element_located((By.CSS_SELECTOR, "a[href*=\"page=packs_add\"][href*=\"cat_id=packs1\"]")))
    elements = self.driver.find_elements(By.XPATH, "//p[contains(.,\'View Packs\')]")
    assert len(elements) == 0
